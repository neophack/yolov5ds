...
class Detect(nn.Module):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                if self.onnx_dynamic or self.grid[i].shape[:3] != x[i].shape[:3]:
                    self.grid[i], self.anchor_grid[i] = self._make_grid(nx, ny, i, bs)
    ...
    def _make_grid(self, nx=20, ny=20, i=0,bs=0):
        d = self.anchors[i].device
        t = self.anchors[i].dtype
        if self.export:
            shape =  self.na, ny, nx, 2
        else:
            shape = bs, self.na, ny, nx, 2  # grid shape
        ...
        if self.export:
            grid = torch.from_numpy((torch.stack((xv, yv), 2).expand(shape) - 0.5).cpu().numpy()).to(d) # add grid offset, i.e. y = 2.0 * x - 0.5
            anchor_grid = torch.from_numpy((self.anchors[i] * self.stride[i]).view((self.na, 1, 1, 2)).expand(shape).cpu().numpy()).to(d) 
        else:
            grid = torch.from_numpy((torch.stack((xv, yv), 2).expand(shape) - 0.5 ).view((bs, self.na, ny, nx, 2)).cpu().numpy()).to(d)  # add grid offset, i.e. y = 2.0 * x - 0.5
            anchor_grid = torch.from_numpy((self.anchors[i] * self.stride[i]).view((1, self.na, 1, 1, 2)).expand(shape).reshape((bs, self.na, ny, nx, 2)).cpu().numpy()).to(d) 
